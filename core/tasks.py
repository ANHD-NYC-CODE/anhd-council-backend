...
from app.mailer import send_update_error_mail, send_update_success_mail, send_general_task_error_mail, send_mail
...
def async_check_notifications_custom_search_daily(self):
    try:
        ...
        for custom_search in custom_searches:
            ...
            new_result_hash = get_query_result_hash(query)
            if past_result_hash != new_result_hash:
                ...
                user_custom_searches = custom_search.usercustomsearch_set.filter(notification_frequency='D')
                if user_custom_searches.exists():
                    for user_custom_search in user_custom_searches:
                        ...
                        if user_custom_search.last_notified_hash != new_result_hash:
                            ...
                            content = "<h3>Hello {}</h3><p>Your saved custom search {} has updated.</p><p>If you believe this email is a mistake please reach out to dapadmin@anhd.org</p><p>- ANHD</p>".format(
                                user.first_name, user_custom_search.name)
                            try:
                                if settings.DEBUG:
    ...
    try:
        ...
        for custom_search in custom_searches:
            ...
            if past_result_hash != new_result_hash:
                ...
                async_update_custom_search_result_hash.delay(custom_search.id)
                # Alert users of the change
                user_custom_searches = custom_search.usercustomsearch_set.filter(notification_frequency='W')
                if user_custom_searches.exists():
                    for user_custom_search in user_custom_searches:
                        # If user hasn't been alerted about this update
                        if user_custom_search.last_notified_hash != new_result_hash:
    ...
    try:
        ...
        for custom_search in custom_searches:
            ...
            if past_result_hash != new_result_hash:
                ...
                async_update_custom_search_result_hash.delay(custom_search.id)
                # Alert users of the change
                user_custom_searches = custom_search.usercustomsearch_set.filter(notification_frequency='M')
                if user_custom_searches.exists():
                    for user_custom_search in user_custom_searches:
                        # If user hasn't been alerted about this update
                        if user_custom_search.last_notified_hash != new_result_hash:
