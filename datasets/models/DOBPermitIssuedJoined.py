...
class DOBPermitIssuedJoined(BaseDatasetModel, models.Model):
    ...
    #     placeholders = ', '.join(["%s" for k in self._meta.get_fields()])
    #     sql = "INSERT INTO {table_name} ({fields}) VALUES ({placeholders}) ON CONFLICT ({primary_key}) DO UPDATE SET {upsert_fields};"
    #     import pdb
    #     pdb.set_trace()
    #     return sql.format(table_name=self._meta.db_table, fields=fields, placeholders=placeholders, primary_key=self._meta.pk.name, upsert_fields=upsert_fields)
